...
class SparkRDS:
    ...
    def upload_features(self, features):
        ...
        df.write.jdbc(url=self.rds_url, table='features', mode='append', properties=self.rds_prop)

    def download_features(self, name, return_type="dict"):
        path = self.get_path(name)
        data = self.spark.read.jdbc(url=self.rds_url, table='features', properties=self.rds_prop)
        data = data.filter(data.http_path == path).collect()[0]
        if return_type == "Row":
            return data
